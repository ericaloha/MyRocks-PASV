...
int wal_write_freq=0;
int total_txn_num=0; 
extern int total_kv_num; 
extern int Active_Flushed;
extern int ForceFlushFlag[8];
extern int global_cf;
...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  WriteThread::WriteGroup write_group; //try to remove entries to cf except _sys_
  ...
  if (!two_write_queues_ || !disable_memtable) {
    ...
    PERF_TIMER_STOP(write_pre_and_post_process_time);
    //Flush(FlushOptions(),) hkc pending
  ...
  if (status.ok()) {
    ...
    if (status.ok()) {
      ...
      if (!parallel) { //hkc may be there has the cf info
        // w.sequence will be set inside InsertInto
        w.status = WriteBatchInternal::InsertInto( //1052813216
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  /*
  else if(UNLIKELY(status.ok() && !single_column_family_mode_ &&
               Active_Flushed==1)){ //after auto flush
    WaitForPendingWrites();
    status = SwitchWAL(write_context);
  ...
  if (UNLIKELY(status.ok() && write_buffer_manager_->ShouldFlush())) {
    ...
    WaitForPendingWrites();
    status = HandleWriteBufferFull(write_context); //all mems broken the threshold of memory usage
  }/*
  else if
  (UNLIKELY(status.ok() && write_buffer_manager_->EnabledShouldFlush()) && Active_Flushed==1){
    //act flush happened, we need to force other cfs to flush 
    WaitForPendingWrites();
    status = HandleWriteBufferFull(write_context);
    Active_Flushed=0;
    for (int i=1;i<6;i++){
      if(ForceFlushFlag[i]!=0){
        Active_Flushed=1;
      }
    }
  }*/
  ...
  if (UNLIKELY(status.ok() && !flush_scheduler_.Empty())) {
    WaitForPendingWrites();//hkc trigger 1
...
/*
int GetLen(uint32_t& klen, char* tmpRep_, int TmpSize,int ptr){
  uint32_t klen=0;
  int size=0;
  int tmpsize=0;
  std::string tmp_len_str;
  if (ptr+8 <TmpSize){
        for (int s=ptr;s<ptr+8;ptr++){
        tmp_len_str[size++]=tmpRep_[s];
        }
        tmpsize=size;
      }else{
        for (int s=ptr;s<TmpSize;s++){
          tmp_len_str[size++]=tmpRep_[s];
        }
        tmpsize=size;
      }
      
      GetVarint32_kc(tmp_len_str,size,&klen);
  return tmpsize-size;
}
*/
/*
...
// write thread. Otherwise this must be called holding log_write_mutex_.  --4 paras
...
IOStatus DBImpl::WriteToWAL(const WriteBatch& merged_batch,
  ...
  *log_size = log_entry.size();
  ...
  /*
  std::ofstream ofs;
  //ofs.open("/home/hkc/log/TEST_sys_WAL.log",std::ios::app);
  ofs.open("/home/hkc/log/TEST_sys.log",std::ios::app);
  ofs << "Batch:[ ";
  for(int i=0;i<(int)merged_batch.GetDataSize();i++){
    //ofs <<"B["; 
    //for(int i=0;i<12;i++){
    ofs << std::hex <<" "<< (int)merged_batch.Data()[i];
  }
  ofs <<"]\n";
  ofs.close();
  */
  ...
  /*
  //hkc test
  if(wal_write_freq<20){
    wal_write_freq++;
  }else
  {
    modified=1;
    for(i;i<13;i++){ //write  sequence: fixed64 count: fixed32
    trimmed[j++]=merged_batch.Data()[i];
    }
  }
  */
  ...
  //hkc test
  //if(global_cf>0 &&(int)merged_batch.Data()[12]==13&&(int)merged_batch.Data()[13]==1){  //d 1    
  if(global_cf<-1000 &&(int)merged_batch.Data()[12]==13&&(int)merged_batch.Data()[13]==1){  
  modified=1;
  for(i;i<13;i++){ //write  sequence: fixed64 count: fixed32
    trimmed[j++]=merged_batch.Data()[i];
  }
  for(i;i<merged_batch.GetDataSize();i++){
    //if ((int) merged_batch.Data()[i] == 13){ //for disable 2pc --> unprepared txn id is null
      //trimmed[j++]=merged_batch.Data()[i];
    //}else 
    if((int)(merged_batch.Data()[i]) == 1){ //value Type
      //for key
      i++;
      uint32_t klen=0;
      ...
      int tmpsize=0;
      std::string tmp_len_str;
      if (i+8 <merged_batch.GetDataSize()){
        for (int s=i;s<i+4;s++){
        tmp_len_str[size++]=merged_batch.Data()[s];
        }
        tmpsize=size;
      }else{
        for (int s=i;s<merged_batch.GetDataSize();s++){
          tmp_len_str[size++]=merged_batch.Data()[s];
        }
        tmpsize=size;
      }
      ...
      GetVarint32_kc(tmp_len_str,size,&klen);
      i+=(tmpsize-size); //bytes occupied by klen
      //int klen=(int)(merged_batch.Data()[i]&0xff);
      i+=klen; //now i points to the first bit after key
      if(klen<0 || klen>1000){
        int x=0;
        //alert!!!
      }
      ...
      GetVarint32_kc(tmp_len_str,size,&vlen);
      i+=(tmpsize-size); //bytes occupied by vlen
      i+=vlen; //now i points to the first bit after value
      if(vlen<0 || vlen>1000){
        int x=0;
        //alert!!!
      }
      ...
      i--;
    }else if((int)(merged_batch.Data()[i]) == 5 ||(int)(merged_batch.Data()[i]) == 6) 
    {
      //[1] kv specify a cf, 0x5 is cf type specifier.
      //[2] in linkbench, it needs the auto increment specifier in sys_ cf for linktable
      //    and the format is the same as 0x5
      if((int) merged_batch.Data()[i+1] != 1){
      i++;  // i points to cf id now
      //for key
      i++; //key len
      int klen=(int)(merged_batch.Data()[i]);
      i+=klen; 
      //for value
      i++; //vlen
      int vlen=(int)(merged_batch.Data()[i]&0xff);
      i+=vlen; //now i points to the last bit of value
      }else // write extra kv for sys cf
      {
        trimmed[j++]=merged_batch.Data()[i]; //5
        ...
        GetVarint32_kc(tmp_len_str,size,&klen);
        int tmpK=i+klen+(tmpsize-size);
        if(tmpK>merged_batch.GetDataSize()){
          tmpK=merged_batch.GetDataSize();
          //alert!!!
        }
        for(i;i<tmpK;i++){
           trimmed[j++]=merged_batch.Data()[i];
        ...
        //for value, now i ponts to the first byte of value (vlen)
        uint32_t vlen=0;
        size=tmpsize=0;
        if (i+8 <merged_batch.GetDataSize()){
          for (int s=i;s<i+8;s++){
        ...
        int tmpV= i+vlen+(tmpsize-size);
        if(tmpV>merged_batch.GetDataSize()){
          tmpV=merged_batch.GetDataSize();
          //alert!!!
        }
        for(i;i<tmpV;i++){
          trimmed[j++]=merged_batch.Data()[i];
        ...
        i--; // the for loop will +1, so -1 here
      }
    }else{
      //for rear that cannot be handled up to now
      for(i;i<merged_batch.GetDataSize();i++){
        trimmed[j++]=merged_batch.Data()[i];
      }
    ...
    ofs <<"4 tri_rep_: ";
    for(int i=0;i< j;i++){
      ofs << std::hex << (int) trimmed[i]<<" ";
    }
    ofs <<std::endl;
    ofs << "Ori WAL Size: " << (int)merged_batch.GetDataSize() <<"; Tri Size: "<<j<<std::endl;
    */
    //ofs << "Ori WAL Size: " << (int)merged_batch.GetDataSize() <<"; Tri Size: "<<j<<std::endl;
  }
  //else
  //{
    ...
    //ofs <<"Ori_rep_: ";
    //for(int m=0;m<merged_batch.GetDataSize();m++){
      //ofs << std::hex << (int)merged_batch.Data()[m]<<" ";
    //}
    //ofs <<std::endl;
  ...
  //ofs << "Ori WAL Size: " << (int) merged_batch.GetDataSize() <<std::endl;
  ...
  //--hkc 
  modified=0; //forbid the opt hkc
  if(modified==1){
    IOStatus io_s = log_writer->AddRecord(Slice(trimmed.c_str(),j));
    if (UNLIKELY(needs_locking)) {
    ...
    alive_log_files_.back().AddSize(j);
    //*log_size = j;
    log_empty_ = false;
    global_cf=-1;
    return io_s;
    ...
    total_log_size_ += log_entry.size(); ///important!!!!!
...
IOStatus DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  /*
  std::ofstream ofs;
  ofs.open("/home/hkc/log/_sys.log",std::ios::app);
  ofs <<"6 Para: ";
  //for(int m=0;m<50;m++){
    //  ofs << std::hex << (int)merged_batch->Data()[m]<<" ";
  //}
  //ofs <<" .... ";
  //for(int m=merged_batch->GetDataSize()-100;m<merged_batch->GetDataSize();m++){
  //    ofs << std::hex << (int)merged_batch->Data()[m]<<" ";
  //}
  ofs <<std::endl;
  ofs.close();
  */
  ...
  uint64_t log_size;
  IOStatus io_s = WriteToWAL(*merged_batch, log_writer, log_used, &log_size);
...
Status DBImpl::ScheduleFlushes(WriteContext* context) {
  ...
  for (auto& cfd : cfds) {
    if (!cfd->mem()->IsEmpty()) {// hkc trigger 2
