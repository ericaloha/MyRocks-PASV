...
Status DBImpl::Get(const ReadOptions& read_options, //breakpoint1
...
Status DBImpl::Get(const ReadOptions& read_options,
  ...
  //added by kc
  /*
  if (column_family->GetID() ==1 ){ //_system_ cf, forbid defaultc cf 
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_sys.log",std::ios::app);
    ofs << "GET["<<(int)column_family->GetID()<<"]: ";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
    ofs <<std::endl;
    ofs.close();
  }else if (column_family->GetID() ==2 ){ //cf_1 cf
    std::ofstream ofs;
    ...
    ofs <<std::endl;
    ofs.close();
  }else if (column_family->GetID() ==3 ){ //cf_1 cf
    std::ofstream ofs;
  ...
  else
  {
    ...
    //ofs << "GET CF[" << column_family->GetID()<<"]::";
    //for(int i=0;i<key.size();i++){
    //  ofs << std::hex << (int) key.data_[i]<<" ";
    //}
    //ofs <<std::endl;
