...
Status DBImpl::Flush(const FlushOptions& flush_options,
  ...
  ROCKS_LOG_INFO(immutable_db_options_.info_log, "[%s] Manual flush start.",
                 cfh->GetName().c_str());
  Status s; //exact flush position
  if (immutable_db_options_.atomic_flush) {
    s = AtomicFlushMemTables({cfh->cfd()}, flush_options,FlushReason::kManualFlush);
  } else { //consider to skip cf_0 (default) //hkc
...
Status DBImpl::Flush(const FlushOptions& flush_options, //hkc key issue
