...
DBOptions SanitizeOptions(const std::string& dbname, const DBOptions& src) {
  ...
  if (result.wal_dir.empty()) {
    ...
    //result.wal_dir = dbname; //hkc wal dir
    //result.wal_dir = "/mnt/WAL_DIR";
    result.wal_dir = "/dev/shm/RDB_WAL";
    //result.wal_dir = "/media/hkc/WALDir";
  }
...
Status DBImpl::Recover(
  ...
  if (s.ok()) {
    ...
    if (!logs.empty()) {
      ...
      bool corrupted_log_found = false; //hkc check sequence
...
Status DBImpl::RecoverLogFiles(const std::vector<uint64_t>& log_numbers,
  ...
  for (auto log_number : log_numbers) {
    ...
    // Open the log file
    std::string fname = LogFileName(immutable_db_options_.wal_dir, log_number);//get wal name
    ...
    std::unique_ptr<SequentialFileReader> file_reader;
    {
      ...
      status = fs_->NewSequentialFile(fname, //open wal
    ...
    log::Reader reader(immutable_db_options_.info_log, std::move(file_reader), //hkc
    ...
    while (!stop_replay_by_wal_filter &&
      ...
      /*
      if((int)record.data_[12]==13 && (int)record.size()>1500){
      redo_records++;
      std::ofstream ofs;
      ofs.open("/home/hkc/log/_wal_redo_simple.log",std::ios::app);
      //ofs << "\n=============================\nRecord: ";
      //for(int i=0;i<record.size();i++){
      //  ofs << std::hex << (int) record.data_[i]<<" ";
      //}
      //ofs <<std::endl;
      ofs <<redo_records<<" ";
      if(redo_records%200==0) ofs <<"\n";
      ofs.close();
      }*/
      
      total_size+=record.size();
      WriteBatchInternal::SetContents(&batch, record); //hkc
      ...
      if (immutable_db_options_.wal_recovery_mode ==
        ...
        if (sequence == *next_sequence) { //*** hkc very important!!! this shit works!!!***/
          stop_replay_for_corruption = false;
        }
        if (stop_replay_for_corruption) {
          logFileDropped(); //hkc!!! thing goes wrong if seq in wal is not consecutive!!!
          break;
        }
      }
      ...
      /*hkc recovery
       [1] for not changing following policy, the kv info should be provided here 
           before inserting into write batch
       [2] the relative kv info should be absorbed from binlog
       [3] we need to:
          (1) get info relative kv pairs info from binlog
          (2) convert them to kv format
      */
      
      //hkc last sequence issue to find mysqlbinlog
  ...
  /*
  std::ofstream ofs;
  ofs.open("/home/hkc/log/wal_size.log",std::ios::app);
  ofs <<total_size<<"\n";
  ofs.close();
  */
...
Status DBImpl::Open(const DBOptions& db_options, const std::string& dbname,
  ...
  *dbptr = nullptr;
  ...
  //hkc recovery time!!!!
  struct timespec tpstart_Recover;
  struct timespec tpend_Recover;
  long timedif_Recover;
  clock_gettime(CLOCK_MONOTONIC, &tpstart_Recover);
  s = impl->Recover(column_families, false, false, false, &recovered_seq);
  clock_gettime(CLOCK_MONOTONIC, &tpend_Recover);
  timedif_Recover = 1000000 * (tpend_Recover.tv_sec - tpstart_Recover.tv_sec) + (tpend_Recover.tv_nsec - tpstart_Recover.tv_nsec) / 1000;
  FILE *debug;
  debug=fopen("/home/hkc/log/debug-WAL.txt","a");
  fprintf(debug, "WAL Recovery Time:%ld \n", timedif_Recover );
  fclose(debug);
  if (s.ok()) {
