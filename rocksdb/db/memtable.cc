...
extern int Total_Flushed;
extern int ForceFlushFlag[8];
extern int pending_cf;
...
bool MemTable::ShouldFlushNow_kc() { //active hkc0126
  //total_txn_num++;
  bool should_flush=0;
  if(flushed_cf_kc[cur_cf]==1 && cur_cf >= 1){
  //if(total_txn_num > 2000){
      //flushed_cf_kc[cur_cf]=0;
      if(total_txn_num != 0){ //hkc bk2
...
void MemTable::UpdateFlushState() {
  //bool HasMemFull=ShouldFlushNow();
  //if(HasMemFull==true){
  //    total_txn_num=2000;   
  //}
  auto state = flush_state_.load(std::memory_order_relaxed);
  if (state == FLUSH_NOT_REQUESTED && ShouldFlushNow()) {
  //if (state == FLUSH_NOT_REQUESTED && ShouldFlushNow_kc()) {
    //Active_Flushed+=1;
...
void MemTable::UpdateFlushState_init() {
  auto state = flush_state_.load(std::memory_order_relaxed);
  if (state == FLUSH_NOT_REQUESTED && ShouldFlushNow()) {
  //if (state == FLUSH_NOT_REQUESTED && ShouldFlushNow_kc()) { //forbid nomal flush
    //Active_Flushed+=1;
    // ignore CAS failure, because that means somebody else requested
    // a flush
    flush_state_.compare_exchange_strong(state, FLUSH_REQUESTED,
...
int MemTable::UpdateFlushState_() { //hkc!!!
  auto state = flush_state_.load(std::memory_order_relaxed);
     //bool HasMemFull=ShouldFlushNow();
     //if(HasMemFull==true){
     //  total_txn_num=2000;
...
void MemTable::UpdateFlushState_kc() {
  if(NeedForceFlushOrNot_kc_()==1){
    auto state = flush_state_.load(std::memory_order_relaxed);
    AfterForceFlush_kc_();
    //passive flush
    flush_state_.compare_exchange_strong(state, FLUSH_REQUESTED,
...
bool MemTable::Add(SequenceNumber s, ValueType type,
  ...
  total_kv_num++;
  std::ofstream ofs;
  //ofs.open("/home/hkc/log/TEST_sys_KV.log",std::ios::app);
  //ofs<<"M["<<cur_cf_test<<" "<<(int)s<<"]"<<std::endl;
  
  //forbid for TEST --hkc 
  // 1.
/*
  ofs.open("/home/hkc/log/TEST_sys.log",std::ios::app);
  ofs << "M CF["<<cur_cf_test<<"] Seq["<< (int)s <<"] Type["<<(int)type<< "] Key[";
