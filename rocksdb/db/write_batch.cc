...
int flushed_cf_kc[6]={0,0,0,0,0,0};
int redo_records=0;
int global_cf=-1;
int try_ACT=0;
int cur_cf_test=0;
//0: not sys_cf, not trigger flush
//1: sys_cf, trigger flush
//2: flush is triggered
int Active_Flushed=0;
int Active_CF=-1;
int Passive_Flushed=0;
int Passive_CF=-1;
int Total_Flushed=7;
int cf_bitmap[8]={0,0,0,0,0,0,0,0};
int ForceFlushFlag[8]={0,0,0,0,0,0,0,0};
int mem_size[6]={0};
int immu_size[6]={0};
int pending_cf=0;
  ...
  while (((s.ok() && !input.empty()) || UNLIKELY(s.IsTryAgain()))) {
    ...
    switch (tag) {
      ...
      case kTypeValue:
        ...
        cur_cf_test=(int)column_family;
        s = handler->PutCF(column_family, key, value);//pending
        if (LIKELY(s.ok())) {
          empty_batch = false;
          found++;
        }
...
Status WriteBatchInternal::Put(WriteBatch* b, uint32_t column_family_id,
  ...
  // ofs <<GetTime_kc_2()<<" PUT["<<(int)column_family_id<<"]: "<<(int)key.size()<<" "<<(int) value.size()<<"\n";
  // ofs.close(); 
  //forbid for TEST //hkc
  //2.
  /*
    std::ofstream ofs;
    ofs.open("/home/hkc/log/kv_sys.log",std::ios::app);
    ofs << column_family_id<<" k[ ";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
    ofs << " ] --> Value[ ";
    for(int i=0;i<value.size();i++){
      ofs << std::hex << (int) value.data_[i]<<" ";
    }
    ofs <<" ] "<<std::endl;
    ofs.close();*/ 
  if(column_family_id==0){ //forbid
    global_cf+=2;
    ...
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_sys.log",std::ios::app);
    //ofs << "PUT["<<(int)column_family_id<<"]: ";
    ofs << "PUT["<<(int)column_family_id<<"]: "<<(int)key.size()<<" "<<(int) value.size()<<"\n";
    ofs.close(); 
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
      if(i>20) {
    ...
    */
  ...
  }else if (column_family_id ==1 ){ //_system_ cf, forbid default cf
    global_cf-=1;
    /*
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_sys.log",std::ios::app);
    ofs << "PUT[1]: BinPos:"<<tmp_pos_kc<<" ";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
    ofs << "--> ";
    ...
    ofs.close();
    */
  }else if (column_family_id ==2 ){ //cf_1 cf
    global_cf+=2;
    /*
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_cf1.log",std::ios::app);
    ofs << "PUT: ";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
    ...
    ofs.close();
    */
  }else if (column_family_id ==3 ){ //cf_2 cf
    global_cf+=2;
    /*
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_cf2.log",std::ios::app);
    ofs << "PUT: ";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
  ...
  else 
  {
    ...
    /*
    std::ofstream ofs;
    ofs.open("/home/hkc/log/_error.log",std::ios::app);
    ofs << "PUT CF[" << column_family_id<<"]::";
    for(int i=0;i<key.size();i++){
      ofs << std::hex << (int) key.data_[i]<<" ";
    }
    ofs << "--> ";
...
class MemTableInserter : public WriteBatch::Handler {
 ...
 public:
  ...
  Status PutCFImpl(uint32_t column_family_id, const Slice& key,
    ...
    pending_cf=(int)column_family_id;
    cur_cf=pending_cf;
    Status seek_status; //hkc check sequence number
    ...
    // an active flush happens, we need to check and find cfs that needs to be flushed forcely
    /*
    if(ForceFlushFlag[(int)column_family_id]>=1 && Active_Flushed==0){//&& Passive_Flushed>=0
             ForceFlushFlag[(int)column_family_id]=0;
             Passive_CF=(int)column_family_id;
             Passive_Flushed+=1;
             mem->ShouldForceFlush_kc_();         
    }else if (try_ACT<200)
    {
      try_ACT++;
    }else if (try_ACT==200)
    {
      try_ACT=0;
      Active_Flushed=0; // it dosent matter if active flushed is 0 before
    }*/
  ...
  Status MergeCF(uint32_t column_family_id, const Slice& key,
    ...
    /*
    if(ForceFlushFlag[(int)column_family_id]>=1 && Active_Flushed==0){// && Passive_Flushed<2
             ForceFlushFlag[(int)column_family_id]=0;
             Passive_CF=(int)column_family_id;
             Passive_Flushed+=1;
             mem->ShouldForceFlush_kc_();         
...
void CheckMemtableFull() { //hkc switch trigger 3
    if (flush_scheduler_ != nullptr) {
      auto* cfd = cf_mems_->current();
      assert(cfd != nullptr);
      uint32_t currentID=cfd->GetID();
      int id_kc=(int)currentID;

      //1.get mutable mem size
      mem_size[id_kc] = cfd->mem()->ApproximateMemoryUsageFast();
      //2. get immutable mem size //hkc0125
      immu_size[id_kc] =cfd->imm()->ApproximateMemoryUsage(); 
      ...
      should_put_mark=1;
      //if(flushed_cf_kc[id_kc]==1 || (cfd->mem()->ShouldScheduleFlush() && cfd->mem()->MarkFlushScheduled())){
        flushed_cf_kc[id_kc]=0; //hkc bk4
        //int cur_mem_size = cfd->mem()->ApproximateMemoryUsageFast();
        //2. get immutable mem size
        //int cur_immu_size = cfd->imm()->ApproximateUnflushedMemTablesMemoryUsage();
        //int cur_immu_size = 0;
        std::ofstream ofs1;
        ofs1.open("/home/hkc/log/memStatus.log",std::ios::app);
        ofs1 <<GetTime_mem_kc()<<"PAS mem size CF[";
        for(int i=0;i<6;i++){
          ofs1 <<mem_size[i]<<" ";
          mem_size[i]=0;
        }
        ...
        ofs1.close();
        /*
        //force flush memtable when a flush happens --hkc
        //actually the formal flush comes to this cycle while forced flush
        //could only enter cf_bitmap check, we will see
        for(int i=0;i<6;i++){
          if(cf_bitmap[i]==1){
            cf_bitmap[i]=2;
          }
        }
        cf_bitmap[currentID]=0;
        */
        // MarkFlushScheduled only returns true if we are the one that
        // should take action, so no need to dedup further
        flush_scheduler_->ScheduleWork(cfd);
      }
      // else if(flushed_cf_kc[id_kc]==1){
      //   int x=cur_cf;
      //   flushed_cf_kc[id_kc]=0;
      //   std::ofstream ofs1;
      //   ofs1.open("/home/hkc/log/memStatus.log",std::ios::app);
      //   ofs1 <<GetTime_mem_kc()<<"ACT mem size CF[";
      //   for(int i=0;i<6;i++){
      //     ofs1 <<mem_size[i]<<" ";
      //     mem_size[i]=0;
      //   }
...
Status WriteBatchInternal::InsertInto(
  ...
  if (next_seq != nullptr) {
    *next_seq = inserter.sequence(); //hkc insert
  }
  if (concurrent_memtable_writes) {
    inserter.PostProcess();
  }
