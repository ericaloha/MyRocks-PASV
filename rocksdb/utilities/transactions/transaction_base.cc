...
Status TransactionBaseImpl::Put(ColumnFamilyHandle* column_family,
  ...
  if (s.ok()) {
    ...
    if(should_put_mark==1){
      rocksdb::Slice mark_key="_sys-mark-hkc_";
      rocksdb::Slice mark_value=rocksdb::Slice(tmp_binlog_kc+" "+std::to_string(tmp_pos_kc)+"kc_break");
      s = GetBatchForWrite()->Put(column_family, mark_key, mark_value);
      if (s.ok()) {
        num_puts_++;
      }
      should_put_mark=2; 
      std::ofstream ofs;
      ofs.open("/home/hkc/log/mark_pos_key.log",std::ios::app);
      ofs <<"CF["<<column_family->GetID()<<"] _sys-mark-hkc_ "<<tmp_binlog_kc+" "+std::to_string(tmp_pos_kc)+"kc_break"<<std::endl;
      ofs.close();
    }
  }
