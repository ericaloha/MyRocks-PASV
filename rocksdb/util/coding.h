...
extern const char* GetVarint32PtrFallback(const char* p,
                                          ...
                                          uint32_t* value);
//hkc                                          
extern const char* GetVarint32PtrFallback_kc(const char* p,
                                          const char* limit,
                                          uint32_t* value);                                          
...
//hkc
inline const char* GetVarint32Ptr_kc(const char* p,
                                  const char* limit,
                                  uint32_t* value) {
  if (p < limit) {
    uint32_t result = *(reinterpret_cast<const unsigned char*>(p));
  ...
  return GetVarint32PtrFallback_kc(p, limit, value);
}
...
inline bool GetVarint32_kc(const std::string input, int& size, uint32_t* value) {
  const char* p = input.data();
  const char* limit = p + size;
  const char* q = GetVarint32Ptr_kc(p, limit, value);
  if (q == nullptr) {
    return false;
  } else {
    size=static_cast<size_t>(limit - q);
    return true;
  }
}
