...
extern float kv_sql_ratio;
extern int flushed_cf_kc[6];
extern int cur_cf;
extern int total_txn_num;
...
static void rocksdb_flush_all_memtables_kc() {
  const Rdb_cf_manager &cf_manager = rdb_get_cf_manager();

  // RocksDB will fail the flush if the CF is deleted,
  // but here we don't handle return status
  for (const auto &cf_handle : cf_manager.get_all_cf()) {
     if(cf_handle.get()->GetID()==2){ //kc0126 modfied, only flush cf4 
       rdb->Flush(rocksdb::FlushOptions(), cf_handle.get());
     }
    //rdb->Flush(rocksdb::FlushOptions(), cf_handle.get());
  }
...
static void rocksdb_report_all_memtables_kc() {
  const Rdb_cf_manager &cf_manager = rdb_get_cf_manager();
...
static int rocksdb_force_flush_memtable_now(
  ...
  rocksdb_flush_all_memtables_kc();
...
class Rdb_transaction_impl : public Rdb_transaction {
 ...
 public:
  ...
  rocksdb::Status put(rocksdb::ColumnFamilyHandle *const column_family,
    ...
    //if(flushed_cf_kc[2]==1&&flushed_cf_kc[3]==1&&flushed_cf_kc[5]==1){
    //cur_cf=column_family->GetID();
    if(total_txn_num==1000){ 
           kv_sql_ratio = total_kv_num / total_txn_num * 1.0;
           total_txn_num=0;
           total_kv_num=0;
          for(int i=1;i<6;i++){ //hkc bk1
            flushed_cf_kc[i]=1;
          }
          total_txn_num++;
          ...
          // flushed_cf_kc[4]=0;
          // flushed_cf_kc[5]=0;
        rocksdb::Slice mark_key="_sys-mark-hkc_";
        std::string mark_value=tmp_binlog_kc+" "+std::to_string(tmp_pos_kc)+"kc_break";
        //should_put_mark=1;
        //rocksdb::Status ss=m_rocksdb_tx->Put(column_family, mark_key, mark_value, assume_tracked);
        // std::ofstream ofs;
        // ofs.open("/home/hkc/log/mark_pos_key.log",std::ios::out);
        // ofs <<"CF["<<cur_cf<<"] ";
        ...
        // ofs<<std::endl;
        // ofs.close();
        // const int char_size=value.size();
        // char tmp_value[char_size]={0};
        // for (size_t i = 0; i < value.size(); i++)
        // {
        //   tmp_value[i]=value.data_[i];
        // }
        // std::string mark_value=tmp_binlog_kc+" "+std::to_string(tmp_pos_kc)+"kc_break"+std::string(tmp_value);
        // const rocksdb::Slice ss=rocksdb::Slice(mark_value);
        // return m_rocksdb_tx->Put(column_family, key, ss, assume_tracked);
    }
    if(should_put_mark==2){
      //rocksdb_flush_all_memtables_kc();//active mode
      should_put_mark=0;
    }
    return m_rocksdb_tx->Put(column_family, key, value, assume_tracked);
    // else if(no_more_cf_5==1 && flushed_cf_kc[2]==1&&flushed_cf_kc[3]==1){
    //       kv_sql_ratio = total_kv_num / total_txn_num * 1.0;
    //       total_txn_num=0;
    //       total_kv_num=0;
    //       cur_cf=4;
...
int ha_rocksdb::update_write_row(const uchar *const old_data,
  ...
  /*
  std::ofstream ofs;
  ofs.open("/home/hkc/log/new_data.log",std::ios::app);
  ofs << "All in all:"<<std::endl;
  ofs << std::hex <<new_data << std::endl;
  ofs << "First 30 bytes:"<<std::endl;
  for(int i=0;i<30;i++){
    ofs << std::hex <<(int)new_data[i]<<" ";
  }
  ofs << std::endl;
  ofs.close();
  */
...
void Rdb_manual_compaction_thread::run() {
  ...
  for (;;) {
    ...
    const auto ret MY_ATTRIBUTE((__unused__)) =
        mysql_cond_timedwait(&m_signal_cond, &m_signal_mutex, &ts); //hkc0125
    if (m_killed) {
      break;
    }
    // make sure, no program error is returned
