...
extern int Global_epoch;
extern int flush_frequency[8];
extern int local_epoch[8];
extern std::string tmp_binlog_kc;
extern uint tmp_pos_kc;
extern int first_cycle[8];
extern int safe_epoch;
extern int flush_flag[8];
extern int cf_bitmap[8];
extern int Passive_Flushed;
extern int ForceFlushFlag[8];
extern int Passive_CF;
extern int Active_Flushed;
extern int Active_CF;
//0:nothing 1: involved 2: waited to be flushe
using std::max;
using std::min;
using std::string;
using std::list;
...
{
  if (likely(thd != NULL))
  { //after inserting a new group of txns, safe pos points to the begining of a set of txn(query,table map, row,xid)
    thd->set_next_event_pos(log_file_name,
                            my_b_tell(&log_file));
    tmp_pos_kc=log_file.pos_in_file;
    tmp_binlog_kc=log_file_name;
  }
}
    ...
    if (likely(length > 0))
    {
      ...
      */
      ...
      while (hdr_offs < length)
      {
        ...
        */
        ...
        else
        {
          ...
          uint event_len=uint4korr(ev + EVENT_LEN_OFFSET);
          uchar *log_pos= ev + LOG_POS_OFFSET;
          if(ev[4]==0x1e){ //hkc binlog **
            //event_len+=3; // netto len
            /*
            int tmp_pos=(int)log_pos[0]+(int)log_pos[1]*256+(int)log_pos[2]*256*256+(int)log_pos[3]*256*256*256;
            tmp_pos+=3;
            
            int tmp_byte=tmp_pos/(256*256*256);
            log_pos[3]=(unsigned char)tmp_byte;
            tmp_pos-=tmp_byte*256*256*256;
            
            tmp_byte=tmp_pos/(256*256);
            log_pos[2]=(unsigned char)tmp_byte;
            tmp_pos-=tmp_byte*256*256;
            
            tmp_byte=tmp_pos/256;
            log_pos[1]=(unsigned char)tmp_byte;
            tmp_pos-=tmp_byte*256;
            
            log_pos[0]=(unsigned char)tmp_pos;
            */
          }
          /* fix end_log_pos */
	  ...
	  if (do_checksum)
          ...
          // for the space of cf and epoch, event+=3
          /* next event header at ... */ /* hkc binlog */ 
          hdr_offs += event_len; // incr by the netto len
          // hkc defined
          //safe_pos=hdr_offs;
          //safe_binlog=log_file.file_name;
  ...
  for (THD *head= first ; head ; head = head->next_to_commit)
  {
    ...
    */
    ...
    if ((head->rli_slave || head->rli_fake) &&
        enable_binlog_hlc && head->hlc_time_ns_next != 0)
    {
                          ...
                          const uchar* extra_row_info)
{
  ...
  */
  ...
  //add cf id, local_epoch here, just have a try hkc binlog ***
  //size_t const len= pack_row(table, table->write_set, row_data, record);
  size_t len= pack_row(table, table->write_set, row_data, record);
  
  std::string tb_l="linktable";
  if (strcmp(table->s->table_name.str,tb_l.c_str())==0){
     cf_bitmap[1]=1;
     cf_bitmap[2]=1;
     cf_bitmap[3]=1;
    row_data[len++]=0x2; //cf_id_2 link pk
    row_data[len++]=(unsigned char)local_epoch[2];
    row_data[len++]=(unsigned char)flush_frequency[2]; //flush frequency during every GE
    
    row_data[len++]=0x3; //cf_id_3 link_sk
    row_data[len++]=(unsigned char)local_epoch[3];
    row_data[len++]=(unsigned char)flush_frequency[3]; //flush frequency during every GE

    row_data[len++]=0x1; //cf_id_1 sys
    row_data[len++]=(unsigned char)local_epoch[1];
    row_data[len++]=(unsigned char)flush_frequency[1]; //flush frequency during every GE
  }
  std::string tb_c="counttable";
  if (strcmp(table->s->table_name.str,tb_c.c_str())==0){
    cf_bitmap[1]=1;
    cf_bitmap[4]=1;
    
    
    row_data[len++]=0x4; //cf_id_2 link pk
    row_data[len++]=(unsigned char)local_epoch[4];
    row_data[len++]=(unsigned char)flush_frequency[4]; //flush frequency during every GE
    
  }
  std::string tb_n="nodetable";
  if (strcmp(table->s->table_name.str,tb_n.c_str())==0){
    cf_bitmap[1]=1;
    cf_bitmap[5]=1;
    
    row_data[len++]=0x5; //cf_id_2 link pk
    row_data[len++]=(unsigned char)local_epoch[5];
    row_data[len++]=(unsigned char)flush_frequency[5]; //flush frequency during every GE
  ...
  for(int i=0;i<8;i++){
    if(flush_flag[i]!=0){
      flush_flag[i]=0;
      std::ofstream ofs;
      if(Passive_Flushed>0&&Passive_CF!=-1){
        ofs.open("/home/hkc/log/safe_pos.log",std::ios::app);
        ofs<<"KV-SQL-Ratio:"<<kv_sql_ratio<<"\t";
        ofs<<"cf_bitmap["<<cf_bitmap[1]<<" "<<cf_bitmap[2]<<" "<<cf_bitmap[3]<<" "<<cf_bitmap[4]<<" "<<cf_bitmap[5]<<"]\t";
        ofs<<"FlushFlag["<<ForceFlushFlag[1]<<" "<<ForceFlushFlag[2]<<" "<<ForceFlushFlag[3]<<" "<<ForceFlushFlag[4]<<" "<<ForceFlushFlag[5]<<"]\t";
        ofs<<"PCT_num["<<Passive_Flushed<<"],PCF["<<Passive_CF<<"];"<<GetTime()<<";CF["<<i<<"];GE["<<Global_epoch<< "];SE["<< safe_epoch << "];SP[" <<tmp_binlog_kc.c_str()<<" --> "<< tmp_pos_kc<<"]"<<std::endl;
        ofs.close();
        Passive_Flushed-=1;
        Passive_CF=-1;
        kv_sql_ratio=0;
      }
      else if(Active_Flushed>0){
        ofs.open("/home/hkc/log/safe_pos.log",std::ios::app);
        ofs<<"KV-SQL-Ratio:"<<kv_sql_ratio<<"\t";
        ofs<<"cf_bitmap["<<cf_bitmap[1]<<" "<<cf_bitmap[2]<<" "<<cf_bitmap[3]<<" "<<cf_bitmap[4]<<" "<<cf_bitmap[5]<<"]\t";
        ofs<<"FlushFlag["<<ForceFlushFlag[1]<<" "<<ForceFlushFlag[2]<<" "<<ForceFlushFlag[3]<<" "<<ForceFlushFlag[4]<<" "<<ForceFlushFlag[5]<<"]\t";
        ofs<<"ACT_num["<<Active_Flushed<<"],"<<"PFlushed["<<Passive_Flushed<<"],ACF,"<<GetTime()<<";CF["<<i<<"];GE["<<Global_epoch<< "];SE["<< safe_epoch << "];SP[" <<tmp_binlog_kc.c_str()<<" --> "<< tmp_pos_kc<<"]"<<std::endl;
        ofs.close();
        //Active_Flushed=0;
        kv_sql_ratio=0;
      }
    ...
    */
  if(first_cycle[safe_epoch]==1 && safe_epoch!=-1){
     ...
     ofs <<"++++++++++++++++++\n"<<GetTime_bin_kc()<<" global epoch:"<<Global_epoch<< "; safe epoch:"<< safe_epoch << "; safe_pos:" <<tmp_binlog_kc.c_str()<<" --> "<< tmp_pos_kc<<std::endl<<"++++++++++++++++++\n";
     ofs.close();
     safe_epoch=-1;
  }
