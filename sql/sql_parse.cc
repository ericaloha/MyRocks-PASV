                      ...
                      uint packet_length)
{
  ...
  //hkc defined
  std::ofstream ofs;
  ofs.open("/home/hkc/log/sql_sys.log",std::ios::app);
  int sql_size=0;
  ofs<<GetTime_kc_3()<<" ";
  for(int i=1;i<4096;i++){
   if((int)thd->raw_query_buffer[i]==0){
     break;
   }
   ofs<< thd->raw_query_buffer[i]<<" ";
   sql_size++;
  }
  ofs<<(int)sql_size<<std::endl;
  ofs.close();
  //forbid 
  ...
  */
  ...
  case SQLCOM_INSERT:
  {
    ...
    */
    ...
    MYSQL_INSERT_START(thd->query());  //hkc
