...
static my_bool opt_skip_verify_if_open= 1;
...
static int encounter_gtid(Gtid cached_gtid)
{
  global_sid_lock->rdlock();
  if (!cached_gtid.empty() && (gtid_set_excluded->ensure_sidno(
          cached_gtid.sidno) != RETURN_STATUS_OK ||
        gtid_set_excluded->_add_gtid(cached_gtid) != RETURN_STATUS_OK))
  {
    global_sid_lock->unlock();
    return 1;
  }
...
int main(int argc, char** argv)
{
  ...
  //hkc binlog recovery time!!!!
  struct timespec tpstart_binlog;
  struct timespec tpend_binlog;
  long timedif_binlog;
  clock_gettime(CLOCK_MONOTONIC, &tpstart_binlog);
  ...
  */
  ...
  clock_gettime(CLOCK_MONOTONIC, &tpend_binlog);
  timedif_binlog = 1000000 * (tpend_binlog.tv_sec - tpstart_binlog.tv_sec) + (tpend_binlog.tv_nsec - tpstart_binlog.tv_nsec) / 1000;
  FILE *debug;
  debug=fopen("/home/hkc/log/debug-BINLOG.txt","a");
  fprintf(debug, "BINLOG Recovery Time:%ld \n", timedif_binlog );
  fclose(debug);
