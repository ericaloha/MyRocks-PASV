...
timer_notify_thread_func(void *arg)
{
  ...
  while (1)
  {
    if (sigwaitinfo(&set, &info) < 0) //hkc0125
      continue;
...
start_helper_thread(void)
{
  ...
  {
    fprintf(stderr, "Failed to initialize pthread barrier. errno=%d", errno);
    return -1;
  }
...
my_timer_initialize(void)
{
  ...
  if (sigfillset(&set))
  {
    fprintf(stderr, "Failed to intialize signal set (errno=%d).", errno);
    return -1;
  }
...
my_timer_initialize(void)
{
  ...
  {
    fprintf(stderr, "Failed to create port (errno= %d).", errno);
    return -1;
  }
